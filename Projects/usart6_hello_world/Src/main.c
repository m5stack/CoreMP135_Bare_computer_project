...
int main(void)
{
  ...
  char *send_msg_data = "heloworld!\n";
  while (1)
  {
    HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
    HAL_USART_Transmit(&husart6, (uint8_t *)send_msg_data, 11, 0xFFFF);
    HAL_Delay(1000);
  }
...
int __write(int file, char *buf, int size)
{
  (void)file;
  (void)size;
  HAL_USART_Transmit(&husart6, (uint8_t *)buf, 1, 0xFFFF);
  return 1;
}
...
GETCHAR_PROTOTYPE
{
  ...
  __HAL_USART_CLEAR_OREFLAG(&husart6);

  HAL_USART_Receive(&husart6, (uint8_t *)&ch, 1, 0xFFFF);
  HAL_USART_Transmit(&husart6, (uint8_t *)&ch, 1, 0xFFFF);
  return ch;
}
